...
<div>
  ...
  {:else if error}
    <div class="text-center py-12">
      ...
      <button onclick={loadStats} class="mt-2 text-sm text-primary-600 hover:underline">Try Again</button>
    </div>
  {:else if stats}
    <div class="space-y-8">
      <!-- Summary Cards -->
            ...
            {#each Object.entries(stats.regionTypeDistribution) as [type, count]}
              ...
              <div>
                ...
                <div class="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-2.5">
                  <div class="h-2.5 rounded-full bg-gradient-to-r from-primary-600 to-accent-600" style="width: {pct}%"></div>
                </div>
              </div>
            {/each}
          </div>
